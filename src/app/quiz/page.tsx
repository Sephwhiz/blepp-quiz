...
export default function QuizPage() {
  ...
  // ✅ MARATHON MENU (no Set B / no coins gate)
...
// ✅ CASE STUDY SESSION (uses local JSON, no file fetch)
if (String(moduleId) === 'case_study' && filePath) {
  // Extract case ID from URL: e.g., filePath = "case-1"
  ...
  // Load case data from public/data/case-studies.json
  ...
  // Pass questions directly to QuizSession
  return (
    <main className="min-h-screen bg-gray-950 relative p-6 max-w-4xl mx-auto pt-20">
      <button
        onClick={() => caseData && router.push(`/quiz?module=case_study&file=case-${caseData.caseId}`)} 
        className="fixed top-4 left-4 z-50 px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white hover:bg-gray-700 transition flex items-center gap-2 shadow-lg"
      >
        ← Back to Cases
      </button>
      ...
      {/* QuizSession with local questions */}
      <QuizSession
        filePath={undefined}
  ...
  // ✅ QUIZ HEADER TITLE (Practice + Marathon)
  ...
  return (
    <main className="min-h-screen bg-gray-950 relative">
      ...
      {headerTitle && (
        ...
        level={levelParam ? parseInt(levelParam) as 1 | 2 : undefined} // ✅ ADD THIS LINE
        domain={domainParam}
        />
    </main>
  )
